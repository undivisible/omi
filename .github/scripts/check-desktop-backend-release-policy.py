...
def _validate_production_secret_bridge(text: str, *, workflow: str) -> list[str]:
    ...
    errors.extend(
        _ordered(
            ...
            (
                "Preflight production desktop secret resource names",
                "Build and push immutable Python images",
            ),
            workflow=workflow,
        )
    )
...
def validate_contract_sources(*, dockerfile: str, desktop_chat: str, pi_extension: str) -> list[str]:
    errors: list[str] = []
    ...
    desktop_contract = "x_omi_chat_contract_version not in {None, '1'}"
    pi_contract = 'OMI_CHAT_CONTRACT_VERSION = "1"'
    if desktop_contract not in desktop_chat or pi_contract not in pi_extension:
        errors.append("desktop chat contract version must remain aligned across Python and Pi")
    return errors
...
def main() -> int:
    errors = validate_all(
        ...
        desktop_chat=(ROOT / "backend/routers/desktop_chat.py").read_text(encoding="utf-8"),
