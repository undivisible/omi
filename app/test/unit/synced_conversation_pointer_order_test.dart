...
ServerConversation _conversation({required DateTime createdAt, DateTime? startedAt, required String id}) {
...
void main() {
  test('offline sync results sort by recording time instead of upload time', () {
    ...
    expect(sorted.map((pointer) => pointer.conversation.id), ['day-2', 'day-1']);
  });

  test('falls back to creation time when recording time is unavailable', () {
    final older = _conversation(id: 'older', createdAt: DateTime.utc(2026, 3, 1));
    final newer = _conversation(id: 'newer', createdAt: DateTime.utc(2026, 3, 2));

    final sorted = sortSyncedConversationPointers([_pointer(older, 0), _pointer(newer, 1)]);

    expect(sorted.map((pointer) => pointer.conversation.id), ['newer', 'older']);
  });
}
