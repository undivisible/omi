...
Wal _wal({required String conversationId, required WalStatus status, required int timerStart}) {
  return Wal(
...
void main() {
  test('filters out synced conversations that still have uploaded/pending WALs', () {
    ...
    final filtered = SyncProvider.filterFullySyncedConversations(conversations: [p1, p2], wals: wals);
  ...
  test('keeps a synced conversation if there are no local WALs yet', () {
    ...
    final filtered = SyncProvider.filterFullySyncedConversations(conversations: [p1], wals: const []);
